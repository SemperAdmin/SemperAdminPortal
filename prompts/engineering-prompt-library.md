...
## Filled Context Block - Semper Admin Portal

Filled 2026-07-05 from package.json and CLAUDE.md. Prepend this block to any mode run against this repository.

```
PROJECT CONTEXT
Stack: TypeScript 5.9 strict, Next.js 16.2.6 App Router with static export, React 19, Tailwind 4.3, MDX content collections validated by zod 4, Pagefind 1.5 search, Vitest 4 tests. No database. Content compiles from content/ into src/generated/ at build time.
Deployment target: GitHub Pages static hosting, built by GitHub Actions. No server runtime, no origin compute.
Load profile: static assets served from the GitHub Pages CDN. Traffic unmeasured. The performance budget lives on the client - bundle size, hydration cost, Pagefind index weight.
