...
export default function ParachuteContent({ parachuteData }: { parachuteData: { factors: Factor[]; rules: Rule[]; docs: { start: Doc[]; stop: Doc[] }; qa: QA[]; references: Ref[] } }) {
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        {tab === "overview" && (
          <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/40">
            <h2 className="text-xl font-semibold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">Parachute Duty Pay (HDIP)</h2>
            <p className="mt-2 text-sm text-zinc-700 dark:text-zinc-300">Monthly hazardous duty incentive pay for Marines assigned to parachute duty billets. Standard parachute duty pays $150/month. Military Free Fall (MFF/HALO) pays $225/month. Only one HDIP may be received per month.</p>
            <div className="mt-4 grid gap-4 sm:grid-cols-3">
              {parachuteData.factors.map((f) => (
            ...
            <div className="mt-6 rounded-2xl bg-[var(--sa-navy)] p-6 text-[var(--sa-cream)]">
              ...
              <p className="mt-2 text-sm opacity-90">$150/month (standard) or $225/month (MFF/HALO) when assigned and performing parachute duties per policy. Only one HDIP may be received per month; pay is tax‑free in designated combat zones.</p>
            </div>
          </section>
        )}
            ...
            <div className="mt-3 space-y-6">
              <div className="rounded-xl border border-black/10 bg-[var(--sa-cream)]/40 p-4 dark:border-white/15 dark:bg-white/10">
                ...
                <ul className="mt-2 space-y-2 text-sm text-zinc-700 dark:text-zinc-300">
                  <li>Assigned by competent orders to a designated parachute duty billet.</li>
                  <li>Hold appropriate qualification (Basic/Jumpmaster/MFF) and maintain currency.</li>
                  <li>Perform qualifying parachute operations per policy.</li>
                </ul>
              </div>
              <div className="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 text-left dark:border-white/15 dark:bg-white/10">
                ...
                <p className="text-xs text-blue-800 dark:text-[var(--sa-cream)]">Qualification alone does not entitle pay. Assignment to a jump billet and monthly performance/currency are required. Temporary inability not due to misconduct may continue pay per policy.</p>
              </div>
            </div>
          </section>
        )}
            ...
            <div className="mt-4 space-y-6">
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {parachuteData.docs.start.map((doc) => (
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {parachuteData.docs.stop.map((doc) => (
                    <div key={doc.name} className="flex items-start gap-4 rounded-xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/15 dark:bg-black/60">
                      <div className="rounded-lg bg-zinc-100 p-3 dark:bg-black/40">
              ...
              <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-6 text-left">
                ...
                <p className="text-xs text-amber-800">Maintain jump logs and monthly certification letters for 24 months for pay audits.</p>
              </div>
            </div>
          </section>
        )}
        ...
        {tab === "process" && (
          <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/40">
            <h2 className="text-xl font-semibold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">The Parachute Pay Pipeline</h2>
              ...
              {["01", "02", "03", "04", "05"].map((step, idx) => (
                <div key={step} className="flex items-center gap-6">
                  ...
                  <div className="flex-1 rounded-2xl border border-black/10 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/60">
                    <h4 className="font-bold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">{["Assignment", "Unit Diary", "Monthly Operations", "Certification", "Payout"][idx]}</h4>
                    <p className="mt-1 text-xs text-zinc-700 dark:text-zinc-300">{[
                      "Receive written orders assigning parachute duty as primary duty (Basic or MFF/HALO).",
                      "S‑1 processes MCTFS entries linking orders to the pay system.",
                      "Perform qualifying parachute operations per policy and maintain currency.",
                      "Command certifies monthly requirements; include jump log/currency verification.",
        ...
        {tab === "troubleshooter" && (
          <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/40">
            ...
            <div className="mt-6 rounded-2xl bg-[var(--sa-red)] p-8 text-white text-center">
              ...
              <p className="mt-2 text-sm opacity-90">Only one type of HDIP may be received per month.</p>
            </div>
          </section>
        )}
        ...
        {tab === "references" && (
          <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/40">
            ...
            <div className="mt-4 grid grid-cols-1 gap-4">
              {parachuteData.references.map((ref) => (
