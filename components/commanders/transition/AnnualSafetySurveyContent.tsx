    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-black/5 bg-gradient-to-r from-[var(--sa-navy)] to-[var(--sa-navy)]/90 p-6 text-white shadow-sm">
        <div className="flex items-center gap-3">
          <ClipboardList className="h-8 w-8 text-[var(--sa-gold)]" />
          <div>
      ...
      {/* Overview */}
      <InfoCard icon={ClipboardList} title="Overview" variant="info">
        <p>{data.overview}</p>
      </InfoCard>
      ...
      <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-black/40">
        ...
        <div className="space-y-3">
          <div className="grid gap-2 text-sm sm:grid-cols-2">
            <div><span className="font-medium">Survey Name:</span> {data.groundUnitsSurvey.name}</div>
            <div><span className="font-medium">Who Takes:</span> {data.groundUnitsSurvey.whoTakes}</div>
            <div className="sm:col-span-2">
              <span className="font-medium">Website:</span>{" "}
              <a href={data.groundUnitsSurvey.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                {data.groundUnitsSurvey.website}
              </a>
            </div>
          </div>
          <div className="mt-3">
            <h3 className="mb-2 font-medium text-zinc-700 dark:text-zinc-300">Annual Cycle:</h3>
            <ul className="space-y-1 pl-4">
              {data.groundUnitsSurvey.annualCycle.map((step, idx) => (
      ...
      {/* Aviation Units Surveys */}
      <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-black/40">
        <h2 className="mb-4 flex items-center gap-2 text-lg font-semibold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">
          <Plane className="h-5 w-5" />
          Aviation Units: CSA, MCAS, ASPA
        </h2>
        <div className="space-y-3">
          {data.aviationUnitsSurveys.surveys.map((survey, idx) => (
            <div key={idx} className="rounded-lg border border-zinc-200 p-3 dark:border-zinc-700">
              <div className="font-medium text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">{survey.name}</div>
              <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Who Takes: {survey.whoTakes}</p>
            </div>
          ))}
          <div className="mt-3 text-sm">
            <span className="font-medium">Website:</span>{" "}
            <a href={data.aviationUnitsSurveys.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
              {data.aviationUnitsSurveys.website}
            </a>
          </div>
          <div className="mt-2 rounded-lg bg-blue-50 p-3 dark:bg-blue-900/20">
            <span className="text-sm text-blue-700 dark:text-blue-300">{data.aviationUnitsSurveys.note}</span>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-black/40">
        <h2 className="mb-4 flex items-center gap-2 text-lg font-semibold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">
          ...
          How to Initiate Survey
        </h2>
        <ol className="space-y-2">
          {data.initiationSteps.map((step, idx) => (
