...
export default function DivePayContent({ diveData }: { diveData: { factors: Factor[]; rules: Rule[]; docs: { start: Doc[]; stop: Doc[] }; qa: QA[]; references: Ref[] } }) {
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        {tab === "overview" && (
          <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/40">
            <h2 className="text-xl font-semibold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">Dive Pay (HDIP)</h2>
            <p className="mt-2 text-sm text-zinc-700 dark:text-zinc-300">Monthly incentive pay for Marines required by competent orders to perform diving as a primary duty. Pay is contingent on maintaining medical clearance and meeting performance requirements.</p>
            <div className="mt-4 grid gap-4 sm:grid-cols-3">
              {diveData.factors.map((f) => (
            ...
            <div className="mt-6 rounded-2xl bg-[var(--sa-navy)] p-6 text-[var(--sa-cream)]">
              ...
              <p className="mt-2 text-sm opacity-90">$150/month, paid when assigned to diving duty and medically cleared. Only one type of HDIP may be received per month.</p>
            </div>
          </section>
        )}
            ...
            <div className="mt-3 space-y-6">
              <div className="rounded-xl border border-black/10 bg-[var(--sa-cream)]/40 p-4 dark:border-white/15 dark:bg-white/10">
                ...
                <ul className="mt-2 space-y-2 text-sm text-zinc-700 dark:text-zinc-300">
                  <li>Assigned by competent orders to a designated diving billet.</li>
                  <li>Maintain current diving physical and medical clearance.</li>
                  <li>Meet minimum diving operations requirements per policy.</li>
                </ul>
              </div>
              <div className="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 text-left dark:border-white/15 dark:bg-white/10">
                ...
                <p className="text-xs text-blue-800 dark:text-[var(--sa-cream)]">Qualification alone does not entitle pay. Assignment and medical clearance are required, and pay stops immediately if clearance expires.</p>
              </div>
            </div>
          </section>
        )}
          ...
          <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/40">
            ...
            <div className="mt-4 space-y-6">
              ...
              <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-6 text-left">
                ...
                <p className="text-xs text-amber-800">Keep assignment orders and medical clearance documentation for 24 months for pay audits.</p>
              </div>
            </div>
          </section>
        )}
        ...
        {tab === "process" && (
          <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/40">
            <h2 className="text-xl font-semibold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">The Dive Pay Pipeline</h2>
              ...
              {["01", "02", "03", "04", "05"].map((step, idx) => (
                <div key={step} className="flex items-center gap-6">
                  ...
                  <div className="flex-1 rounded-2xl border border-black/10 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/60">
                    <h4 className="font-bold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">{["Assignment", "Unit Diary", "Operations", "Medical & Certification", "Payout"][idx]}</h4>
                    <p className="mt-1 text-xs text-zinc-700 dark:text-zinc-300">{[
                      "Receive written orders assigning diving as primary duty.",
                      "S‑1 processes MCTFS entries linking orders to pay system.",
                      "Conduct qualifying diving operations per policy.",
                      "Maintain current diving physical; command certifies duty each month.",
                      "Appears on LES 1–2 pay periods after certification.",
                    ][idx]}</p>
                  </div>
                </div>
              ))}
