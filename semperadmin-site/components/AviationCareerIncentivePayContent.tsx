...
type System = { title: string; points: string[] };
type Milestone = { checkpoint: string; requirement: string; result: string };
type Definition = { term: string; desc: string };
type Conditional = { title: string; bullets: string[] };

export default function AviationCareerIncentivePayContent({ acipData }: { acipData: { intro: string; factors: Factor[]; rules: Rule[]; systems?: System[]; milestones?: Milestone[]; definitions?: Definition[]; conditional?: Conditional[]; docs: { start: Doc[]; stop: Doc[] }; qa: QA[]; references: Ref[] } }) {
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        {tab === "overview" && (
          <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/40">
            <h2 className="text-xl font-semibold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">Flight Pay – Aviation Career Incentive Pay (ACIP)</h2>
            <p className="mt-2 text-sm text-zinc-700 dark:text-zinc-300">{acipData.intro}</p>
            <div className="mt-4 grid gap-4 sm:grid-cols-3">
              {acipData.factors.map((f) => (
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/60">
                ...
                <div className="mt-2 text-sm text-zinc-700 dark:text-zinc-300">Varies by Years of Aviation Service (YAS) and gate status.</div>
              </div>
              <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/60">
            ...
            <div className="mt-6 rounded-2xl bg-[var(--sa-navy)] p-6 text-[var(--sa-cream)]">
              ...
              <p className="mt-2 text-sm opacity-90">Monthly incentive pay for aeronautically designated officers on operational flying duty. Rates follow a YAS gate schedule. Requires current flight orders, medical qualification, and assignment to operational flying.</p>
            </div>
            {acipData.systems && acipData.systems.length > 0 && (
              <div className="mt-6">
                <h3 className="text-xs font-black uppercase text-zinc-600 dark:text-zinc-400 tracking-widest mb-3">The ACIP Gate System</h3>
                <div className="grid gap-4 sm:grid-cols-2">
                  {acipData.systems.map((s) => (
                    <div key={s.title} className="rounded-2xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/15 dark:bg-black/60">
                      <div className="text-lg font-bold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">{s.title}</div>
                      <ul className="mt-2 space-y-2 text-xs text-zinc-700 dark:text-zinc-300">
                        {s.points.map((p, i) => (
                          <li key={`${s.title}-${i}`}>{p}</li>
                        ))}
                      </ul>
                    </div>
                  ))}
            ...
            {acipData.milestones && acipData.milestones.length > 0 && (
              <div className="mt-6">
                <h3 className="text-xs font-black uppercase text-zinc-600 dark:text-zinc-400 tracking-widest mb-3">Gate Milestones</h3>
                <div className="grid gap-4 sm:grid-cols-2">
                  {acipData.milestones.map((m) => (
                    <div key={m.checkpoint + m.requirement} className="rounded-2xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/15 dark:bg-black/60">
                      <div className="text-sm font-semibold text-zinc-600 dark:text-zinc-400">{m.checkpoint}</div>
                      <div className="mt-1 text-[var(--sa-navy)] dark:text-[var(--sa-cream)] font-bold">{m.requirement}</div>
                      <div className="mt-1 text-xs text-zinc-700 dark:text-zinc-300">{m.result}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-3 space-y-6">
              <div className="rounded-xl border border-black/10 bg-[var(--sa-cream)]/40 p-4 dark:border-white/15 dark:bg-white/10">
                ...
                <ul className="mt-2 space-y-2 text-sm text-zinc-700 dark:text-zinc-300">
                  <li>Aeronautically designated officer.</li>
                  <li>Assigned to operational flying duty.</li>
                  <li>Current flight orders on file.</li>
                  <li>Current flight physical and medical qualification.</li>
                  <li>Meets YAS gate and service requirements.</li>
                </ul>
              </div>
              <div className="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 text-left dark:border-white/15 dark:bg-white/10">
                ...
                <p className="text-xs text-blue-800 dark:text-[var(--sa-cream)]">Assignment and current flight orders drive eligibility. Rates depend on YAS gates and may change with career status.</p>
              </div>
              {acipData.definitions && acipData.definitions.length > 0 && (
                <div className="rounded-xl border border-black/10 bg-white p-4 dark:border-white/15 dark:bg-black/60">
                  <h3 className="font-bold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">Critical Definitions</h3>
                  <ul className="mt-2 space-y-2 text-sm text-zinc-700 dark:text-zinc-300">
                    {acipData.definitions.map((d) => (
                      <li key={d.term}><span className="font-semibold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">{d.term}:</span> {d.desc}</li>
                    ))}
                  </ul>
                </div>
              )}
          ...
          <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/40">
            ...
            <div className="mt-4 space-y-6">
              ...
              <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-6 text-left">
                ...
                <p className="text-xs text-amber-800">Retain flight orders, medical, and assignment documentation for 24 months for pay audits.</p>
              </div>
            </div>
          </section>
        )}
        ...
        {tab === "process" && (
          <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/40">
            <h2 className="text-xl font-semibold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">The ACIP Pay Pipeline</h2>
              ...
              {["01", "02", "03", "04", "05", "06"].map((step, idx) => (
                <div key={step} className="flex items-center gap-6">
                  ...
                  <div className="flex-1 rounded-2xl border border-black/10 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/60">
                    <h4 className="font-bold text-[var(--sa-navy)] dark:text-[var(--sa-cream)]">{["Aeronautical Designation", "Flight Physical", "Issue Flight Orders", "Begin Operational Flying", "Unit Diary Start", "LES Verification"][idx]}</h4>
                    <p className="mt-1 text-xs text-zinc-700 dark:text-zinc-300">{[
                      "Receive aeronautical designation and wings per service authority.",
                      "Complete and maintain a current flight physical and medical qualification.",
                      "Command issues flight orders for operational flying duty.",
                      "Report to billet and perform operational flying duties.",
                      "S‑1 processes MCTFS entries to start ACIP based on orders.",
                      "Verify ACIP appears on LES within 1–2 pay periods after processing.",
                    ][idx]}</p>
                  </div>
                </div>
              ))}
            </div>
            {acipData.conditional && acipData.conditional.length > 0 && (
              <div className="mt-6 space-y-4">
                {acipData.conditional.map((c) => (
                  <div key={c.title} className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-left">
                    <h4 className="font-bold text-amber-800">{c.title}</h4>
                    <ul className="mt-2 space-y-2 text-xs text-amber-800">
                      {c.bullets.map((b, i) => (
                        <li key={`${c.title}-${i}`}>{b}</li>
                      ))}
                    </ul>
                  </div>
                ))}
        ...
        {tab === "troubleshooter" && (
          <section className="rounded-xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/15 dark:bg-black/40">
            ...
            <div className="mt-6 rounded-2xl bg-[var(--sa-red)] p-8 text-white text-center">
              <h3 className="text-xl font-bold">ACIP and HDIP</h3>
              <p className="mt-2 text-sm opacity-90">ACIP may be received in addition to HDIP when independently eligible.</p>
            </div>
          </section>
        )}
