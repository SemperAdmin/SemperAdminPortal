...
// Fetch with AbortController-based timeout (browser-compatible)
...
// Resolve the best reachable proxy base by checking /health
async function resolveProxyBase() {
  const deployed = "https://usmc-directives-proxy.onrender.com";
  const localCandidates = ["http://localhost:3005", "http://localhost:3000"];
  const host = typeof window !== 'undefined' ? window.location.hostname : '';
  const candidates = (host === 'localhost' || host === '127.0.0.1')
    ? [...localCandidates, deployed]
    : [deployed, ...localCandidates];

  for (const base of candidates) {
    try {
      const res = await fetchWithTimeout(`${base}/health`, 3000);
      if (res && res.ok) {
        CUSTOM_PROXY_URL = base;
        console.log('[Proxy] Selected base URL:', base);
        return base;
      }
    } catch (_) {}
  }

  console.warn('[Proxy] No proxy reachable; will rely on direct/CORS fallbacks');
  CUSTOM_PROXY_URL = '';
  return null;
}
...
function initTabKeyboardNavigation() {
  const buttons = Array.from(messageTypeButtons);
  buttons.forEach((btn, idx) => {
    btn.addEventListener('keydown', (e) => {
      const key = e.key;
      let nextIdx = null;
      if (key === 'ArrowRight') { nextIdx = (idx + 1) % buttons.length; }
      else if (key === 'ArrowLeft') { nextIdx = (idx - 1 + buttons.length) % buttons.length; }
...
function switchMessageType(type) {
  ...
  filterMessages();
  if (resultsDiv) { resultsDiv.focus(); }
}
...
function updateResultsCount() {
  ...
  const savedRange = dateRangeSelect.value;
  const rangeText = savedRange === '0' ? 'All time' :
                    savedRange === '1' ? 'Today' :
                    savedRange === '7' ? 'Last 7 days' :
                    savedRange === '14' ? 'Last 14 days' :
                    savedRange === '30' ? 'Last 30 days' :
                    savedRange === '60' ? 'Last 60 days' :
                    savedRange === '90' ? 'Last 90 days' : `${savedRange} days`;
  const term = searchInput.value.trim();
  const termText = term ? ` • “${term}”` : '';
  statusDiv.innerHTML = `${countText} • ${rangeText}${termText}`;
}
...
feedbackForm.addEventListener('submit', async (e) => {
  ...
  try {
    // Send to backend
    const apiBase = CUSTOM_PROXY_URL || '';
...
function resetFilters() {
  searchInput.value = "";
  dateRangeSelect.value = "0";
  quickFilterButtons.forEach(btn => {
    btn.classList.remove('active');
    btn.setAttribute('aria-pressed','false');
  });
  localStorage.setItem('filter_date_range', '0');
  switchMessageType('all');
  filterMessages();
}
